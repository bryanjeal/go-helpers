...
PACKAGE = github.com/bryanjeal/go-helpers

.PHONY: vendor check fmt lint test test-race vet test-cover-html help
.DEFAULT_GOAL := help

vendor: ## Install govendor and sync helpers's vendored dependencies
...
fmt: ## Run gofmt linter
	@for d in `govendor list -no-status +local | sed 's/github.com.bryanjeal.go-helpers/./'` ; do \
...
test-cover-html: PACKAGES = $(shell govendor list -no-status +local | sed 's/github.com.bryanjeal.go-helpers/./')
